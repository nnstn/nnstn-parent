package com.pursuit.hadoop.hdfs;
...
public class HdfsClient {
	...
	public void init() throws Exception {
		...
		conf.set("fs.defaultFS", "hdfs://172.21.10.185:9000");
		/**
...
//		 fs = FileSystem.get(conf);
		...
		fs = FileSystem.get(new URI("hdfs://172.21.10.185:9000"), conf, "redis");
	...
	public void testAddFileToHdfs() throws Exception {
		...
		// 要上传的文件所在的本地路径
		Path src = new Path("f:/wangjn1130-cachecloud-master.zip");
		// 要上传到hdfs的目标路径
		Path dst = new Path("/");
...
//		fs.copyToLocalFile(new Path("/apache-flume-1.6.0-bin.tar.gz"), new Path("d:/"));
		fs.copyToLocalFile(false,new Path("/wangjn1130-cachecloud-master.zip"), new Path("d:/"),true);
		fs.close();
